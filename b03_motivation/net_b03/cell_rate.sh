#!/usr/bin/env bash
# Set the shared-link rate for one cell (B03 copy; container b03-gateway).
# The parent class 1:1 IS the shared link (rate = sig-bit).  Signaling
# (class 1:10, dst port 9703) is guaranteed half and may borrow to the full
# link; background (class 1:20, iperf3 port 5201) likewise.  With no
# background traffic signaling gets the full rate; with saturating background
# it falls back toward its guarantee.
set -euo pipefail
SIG_BIT=""
while (($#)); do
  case "$1" in
